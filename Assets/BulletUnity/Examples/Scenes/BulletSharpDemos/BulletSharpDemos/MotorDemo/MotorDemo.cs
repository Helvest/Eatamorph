...
namespace MotorDemo
{
	internal class MotorDemo : Demo
	{
		private Vector3 eye = new Vector3(8, 4, 8);
		private Vector3 target = new Vector3(0, 0, 0);
		private List<TestRig> rigs = new List<TestRig>();
		private float m_Time;
		private float fCyclePeriod;
		private float fMuscleStrength;
		private const int NumLegs = 6;
		private const int BodyPartCount = 2 * NumLegs + 1;
		private const int JointCount = BodyPartCount - 1;

		private class TestRig
		{
			private DynamicsWorld ownerWorld;
			private CollisionShape[] shapes = new CollisionShape[BodyPartCount];
			private RigidBody[] bodies = new RigidBody[BodyPartCount];
			private TypedConstraint[] joints = new TypedConstraint[JointCount];
			...
			public TestRig(DynamicsWorld ownerWorld, Vector3 positionOffset, bool bFixed)
			{
				...
				var offset = Matrix.Translation(positionOffset);

				// root
				var vRoot = new Vector3(0, fHeight, 0);
				var transform = Matrix.Translation(vRoot);
				if (bFixed)
				{
				...
				for (i = 0; i < NumLegs; i++)
				{
					...
					var vBoneOrigin = new Vector3(fCos * (fBodySize + 0.5f * fLegLength), fHeight, fSin * (fBodySize + 0.5f * fLegLength));

					// thigh
					var vToBone = (vBoneOrigin - vRoot);
					vToBone.Normalize();
					var vAxis = Vector3.Cross(vToBone, vUp);
		...
		{
			var rig = new TestRig(World, startOffset, isFixed);
			rigs.Add(rig);
		}
		...
		private void SetMotorTargets(float deltaTime)
		{
			...
			foreach (var rig in rigs)
			{
				...
				{
					var hingeC = rig.GetJoints()[i] as HingeConstraint;
