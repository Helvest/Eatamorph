...
{
	internal class ConvexDecompositionDemo : Demo
	{
		private Vector3 eye = new Vector3(35, 10, 35);
		private Vector3 target = new Vector3(0, 5, 0);
		private Vector3 convexDecompositionObjectOffset;
		private List<TriangleMesh> trimeshes = new List<TriangleMesh>();
		...
		private void MyContactCallback(ManifoldPoint cp, CollisionObjectWrapper colObj0Wrap, int partId0, int index0, CollisionObjectWrapper colObj1Wrap, int partId1, int index1)
		{
			...
			{
				var compound = colObj0Wrap.CollisionObject.CollisionShape as CompoundShape;
				compound.GetChildShape(index0);
			}

			if (colObj1Wrap.CollisionObject.CollisionShape.ShapeType == BroadphaseNativeType.CompoundShape)
			{
				var compound = colObj1Wrap.CollisionObject.CollisionShape as CompoundShape;
		...
		protected override void OnInitializePhysics()
		{
			...
			var bytes = (UnityEngine.TextAsset)UnityEngine.Resources.Load("file.obj");
			...
			for (i = 0; i < tcount; i++)
			{
				...
				var vertex0 = vertices[index0] * localScaling;
				var vertex1 = vertices[index1] * localScaling;
				var vertex2 = vertices[index2] * localScaling;
			...
			{
				var centroid = convexDecomposition.convexCentroids[i];
				var convexShape2 = convexDecomposition.convexShapes[i];
				var trans = Matrix.Translation(centroid);
				if (sEnableSAT)
				{
