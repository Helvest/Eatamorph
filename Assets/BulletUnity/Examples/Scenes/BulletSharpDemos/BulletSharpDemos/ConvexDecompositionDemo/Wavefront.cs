...
namespace ConvexDecompositionDemo
{
	...
	{
		private readonly char[] faceSplitSchars = { '/' };
		private readonly char[] lineSplitChars = { ' ' };
		...
		private Vector3 ToVector3(string f0, string f1, string f2)
		{
			return new Vector3(
				float.Parse(f0, CultureInfo.InvariantCulture),
				float.Parse(f1, CultureInfo.InvariantCulture),
		...
		private int GetVertex(string[] faceVertex)
		{
			...
			var position = vertices[vindex - 1];
		...
		public List<Vector3> Vertices => finalVertices;
	}
}
