...
namespace DemoFramework
{
	...
	public class MeshFactory
	{
		...
		public static UnityEngine.Vector3[] CreateCapsule(CapsuleShape shape, out int[] indices)
		{
			...
			var vertices = new UnityEngine.Vector3[vertexCount * 2];
			...
			var cylinderOffset = GetVectorByAxis(0, -cylinderHalfHeight, 0, up);
		...
		public static UnityEngine.Vector3[] CreateConvexHull(ConvexHullShape shape)
		{
			...
			{
				var v0 = points[(int)indices[i]].ToUnity();
				var v1 = points[(int)indices[i + 1]].ToUnity();
				var v2 = points[(int)indices[i + 2]].ToUnity();
		...
		{
			var allVertices = new List<UnityEngine.Vector3[]>();
			...
			{
				var sphereVertices = CreateSphere(shape.GetSphereRadius(i), out int[] sphereIndices);

				// Adjust sphere position
				var position = shape.GetSpherePosition(i).ToUnity();
			...
			var finalVertices = new UnityEngine.Vector3[vertexCount];
			int vo = 0;
		...
		private static UnityEngine.Vector3[] CreatePolyhedralConvexShape(PolyhedralConvexShape polyhedralConvexShape, out int[] indices)
		{
			...
			var vertices = new UnityEngine.Vector3[numVertices * 3];
			for (int i = 0; i < numVertices; i += 4)
			{
				polyhedralConvexShape.GetVertex(i, out var v0);
				polyhedralConvexShape.GetVertex(i + 1, out var v1);
				polyhedralConvexShape.GetVertex(i + 2, out var v2);
				polyhedralConvexShape.GetVertex(i + 3, out var v3);

				var v01 = (v0 - v1).ToUnity();
				var v02 = (v0 - v2).ToUnity();
		...
		private static UnityEngine.Vector3[] CreateSphere(float radius, out int[] indices)
		{
			int slices = (int)(radius * 10.0f);
			int stacks = (int)(radius * 10.0f);
			slices = (slices > 16) ? 16 : (slices < 3) ? 3 : slices;
		...
		public static UnityEngine.Vector3[] CreateStaticPlane(StaticPlaneShape shape, out int[] indices)
		{
			...
			PlaneSpace1(shape.PlaneNormal, out var vec0, out var vec1);
		...
		private static UnityEngine.Vector3[] CreateTriangleMesh(StridingMeshInterface meshInterface)
		{
