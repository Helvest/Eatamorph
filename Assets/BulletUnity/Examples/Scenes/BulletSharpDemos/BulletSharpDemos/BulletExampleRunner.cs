...
public class BulletExampleRunner : MonoBehaviour
{
	...
	private bool showDemoNames = false;
	public GUILayoutOption w = GUILayout.MinWidth(150);
	private int maxPerCol = 15;
	...
	public static BulletExampleRunner Get()
	{
		if (singleton == null)
		{
			var ws = FindObjectsOfType<BulletExampleRunner>();
			if (ws.Length == 1)
			{
				singleton = ws[0];
			}
	...
	public void PostOnInitializePhysics()
	{
		...
		{
			var co = demo.World.CollisionObjectArray[i];
			var cs = co.CollisionShape;
			GameObject go;
			if (cs.ShapeType == BroadphaseNativeType.SoftBodyShape)
			{
				var sb = (BulletSharp.SoftBody.SoftBody)co;
				if (sb.Faces.Count == 0)
				{
					//rope
			...
			else
			{
				...
				if (cs.ShapeType == BroadphaseNativeType.CompoundShape)
				{
					...
					var rbp = go.AddComponent<BulletRigidBodyProxy>();
					rbp.target = co as RigidBody;
					foreach (var child in (cs as CompoundShape).ChildList)
					{
						var childTransform = child.Transform;
						var ggo = new GameObject(child.ToString());
						var mf = ggo.AddComponent<MeshFilter>();
						var m = mf.mesh;
						MeshFactory2.CreateShape(child.ChildShape, m);
						var mr = ggo.AddComponent<MeshRenderer>();
						mr.sharedMaterial = mat;
						ggo.transform.SetParent(go.transform);
						var mt = childTransform.ToUnity();
				...
				{
					var css = (CapsuleShape)cs;
					var ggo = GameObject.CreatePrimitive(PrimitiveType.Capsule);
	...
	private void Update()
	{
		if (demo.Input != null)
		{
			...
			{
				var k = BulletSharpExamples.Input.UnityKeys[i];
	...
	public GameObject CreateUnityCollisionObjectProxy(CollisionObject body)
	{
		...
		var go = new GameObject(body.ToString());
	...
	public GameObject CreateUnitySoftBodyRope(BulletSharp.SoftBody.SoftBody body)
	{
		...
		var ropeProxy = rope.GetComponent<BulletRopeProxy>();
	...
	public GameObject CreateUnitySoftBodyCloth(BulletSharp.SoftBody.SoftBody body)
	{
		...
		var node2vertIdx = new Dictionary<BulletSharp.SoftBody.Node, int>();
		...
		var go = Instantiate(softBodyPrefab);
		var sbp = go.GetComponent<BulletSoftBodyProxy>();
		var trisRev = new List<int>();
	...
	public void CreateUnityMultiBodyLinkColliderProxy(MultiBodyLinkCollider body)
	{
		...
		var cs = body.CollisionShape;
		if (cs is BoxShape)
		{
			var bxcs = cs as BoxShape;
			var s = bxcs.HalfExtentsWithMargin;
			var mr = cube.GetComponentInChildren<MeshRenderer>();
			mr.transform.localScale = s.ToUnity() * 2f;
			var m = body.WorldTransform.ToUnity();
			...
			var cp = cube.AddComponent<BulletMultiBodyLinkColliderProxy>();
			cp.target = body;
