...
namespace ConcaveRaycastDemo
{
	...
	internal class RaycastBar
	{
		...
		private Vector3[] hit = new Vector3[NUMRAYS_IN_BAR];
		...
		public void Draw(IDebugDraw drawer)
		{
			...
			for (i = 0; i < NUMRAYS_IN_BAR; i++)
			{
				var to = hit[i] + normal[i];
				drawer.DrawLine(ref hit[i], ref to, ref white);
			}
		}
	}

	internal class ConcaveRaycastDemo : Demo
	{
		...
		protected override void OnInitializePhysics()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				var startTransform = Matrix.Translation(2 * i, 10, 1);
