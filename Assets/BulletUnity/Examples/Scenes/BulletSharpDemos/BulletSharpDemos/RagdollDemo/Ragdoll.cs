...
namespace RagdollDemo
{
	...
	internal class Ragdoll
	{
		...
		private CollisionShape[] shapes = new CollisionShape[(int)BodyPart.Count];
		private RigidBody[] bodies = new RigidBody[(int)BodyPart.Count];
		private TypedConstraint[] joints = new TypedConstraint[(int)Joint.Count];
