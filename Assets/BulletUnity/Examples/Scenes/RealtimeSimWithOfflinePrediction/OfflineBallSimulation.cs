...
public static class OfflineBallSimulation
{
	...
	{
		var ballPositions = new List<Vector3>(numberOfSimulationSteps);
		...
		var bw = WorldsManager.WorldController;
		bw.CreatePhysicsWorld(out var cw, out var CollisionConf, out var Dispatcher, out var Broadphase, out var Solver, out var softBodyWorldInfo);
		...
		var sourceWorld = bw.DDWorld;
		RigidBody bulletBallRb = null;
		var mm = BulletSharp.Math.Matrix.Identity;
		for (int i = 0; i < sourceWorld.NumCollisionObjects; i++)
		{
			var co = sourceWorld.CollisionObjectArray[i];
			if (co != null && co.UserObject is BRigidBody)
			{
				var rb = (BRigidBody)co.UserObject;
				var existingShape = rb.GetComponent<BCollisionShape>();
				...
				var localInertia = new BulletSharp.Math.Vector3();
				rb.CreateOrConfigureRigidBody(ref bulletRB, ref localInertia, shape, null);
				var pos = rb.GetCollisionObject().WorldTransform.Origin;
				var rot = rb.GetCollisionObject().WorldTransform.GetOrientation();
