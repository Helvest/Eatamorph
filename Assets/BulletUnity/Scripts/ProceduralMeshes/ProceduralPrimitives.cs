...
namespace BulletUnity
{
	...
	public static class ProceduralPrimitives
	{
		...
		{
			var mesh = CreateMeshBox(sides, sides, sides);
			mesh.name = "PPCube";
	...
	3 + 4 * 5, 2 + 4 * 5, 1 + 4 * 5,
		...
		{
			var mesh = CreateMeshCone(height, radius, radius, nbSides);
		...
		{
			var mesh = CreateMeshCone(height, baseSize, 0f, 4);
			mesh.name = "PPPyramid";
		...
		public static void CreateMeshCapsule(Mesh mesh, float height = 1, float bottomRadius = 1f, float topRadius = 0f, int nbSides = 18, int axis = 1)
		{
			...
			var offset = new Vector3(0f, -1f, 0f) * (height / 2);
        ...
        */
		...
		public static Mesh BuildMeshFromData(float[] vertices, int[] triangles)
		{
			...
			var newVerts = new Vector3[numVertices];
