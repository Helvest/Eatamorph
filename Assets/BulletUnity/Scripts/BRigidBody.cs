...
namespace BulletUnity
{
	...
	public class BRigidBody : BCollisionObject, IDisposable
	{
		...
		{
			get => (RigidBody)collisionObject;
			set => collisionObject = value;
		}

		public RigidBody RigidBody => (RigidBody)collisionObject;

		private Vector3B _localInertia = Vector3B.Zero;
		public Vector3B localInertia => _localInertia;

		public override BulletSharp.CollisionFlags collisionFlags
		{
			get => m_collisionFlags;
			set
			{
		...
		internal override bool _BuildCollisionObject()
		{
			...
			var rb = (RigidBody)collisionObject;
		...
		protected override void Awake()
		{
			...
			var rbs = GetComponentsInParent<BRigidBody>();
			if (rbs.Length != 1)
			{
		...
		protected override void OnDisable()
		{
			if (m_rigidBody != null && isInWorld)
			{
				...
				{
					var btc = (BTypedConstraint)m_rigidBody.GetConstraintRef(i).Userobject;
		...
		protected override void Dispose(bool isdisposing)
		{
			if (isInWorld && isdisposing && m_rigidBody != null)
			{
				...
				if (pw?.DDWorld != null)
				{
					...
					{
						var tc = (BTypedConstraint)m_rigidBody.GetConstraintRef(i - 1).Userobject;
		...
		public bool CreateOrConfigureRigidBody(ref RigidBody rb, ref Vector3B localInertia, CollisionShape cs, MotionState motionState)
		{
			...
			if (rb == null)
			{
				...
				var rbInfo = new RigidBodyConstructionInfo(bulletMass, motionState, cs, localInertia)
				{
					Friction = _friction,
				...
				if (motionState == null)
				{
					var rotation = transform.rotation.ToBullet();
					var position = transform.position.ToBullet();
