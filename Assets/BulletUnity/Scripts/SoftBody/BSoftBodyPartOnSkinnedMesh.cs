...
public class BSoftBodyPartOnSkinnedMesh : BSoftBody
{
	...
	private MeshFilter physicsSimMesh;
	...
	public void BindBonesToSoftBodyAndNodesToAnchors()
	{
		...
		var m = physicsSimMesh.sharedMesh;
		...
		var foundMatches = new List<BoneAndNode>(bones.Length);
	...
	{
		var aa = new Edge(p, a, ns, vs);
		var bb = new Edge(p, b, ns, vs);
		if (!edges.Contains(aa))
		{
			edges.Add(aa);
		}
	...
	public void OnDrawGizmosSelected()
	{
		...
		if (debugDisplayMappedAnchors)
		{
			...
			for (int i = 0; i < anchors.Length; i++)
			{
				...
				{
					var pos = transform.TransformPoint(anchors[i].anchorPosition[j]);
	...
	internal override bool _BuildCollisionObject()
	{
		...
		var mesh = physicsSimMesh.sharedMesh;
		...
		var mr = physicsSimMesh.GetComponent<MeshRenderer>();
		if (mr != null)
		{
    ...
    */
	...
	public void LateUpdate()
	{
		if (isInWorld)
		{
			...
			for (int i = 0; i < bone2idxMap.Length; i++)
			{
				...
				var edgeXnorm = Vector3.Cross(verts[bn.edges[0].nodeIdx] - verts[bn.nodeIdx], norms[bn.nodeIdx]);
				edgeXnorm.Normalize();

				var q = WahbasSolution(bn.bindNormal, bn.edges[0].bindEdgeXnorm, norms[bn.nodeIdx], edgeXnorm);
			...
			if (debugDisplaySimulatedMesh)
			{
				if (myMesh == null)
				{
					...
					var mf = physicsSimMesh.GetComponent<MeshFilter>();
	...
	{
		var r1Xr2 = Vector3.Cross(r1, r2);
		var b1Xb2 = Vector3.Cross(b1, b2);
		float b1Dotr1 = Vector3.Dot(b1, r1);
		var b1Xr1 = Vector3.Cross(b1, r1);
		float mu = (1 + b1Dotr1) *
		...
		var q = new Quaternion(qV.x, qV.y, qV.z, qs);
		return q;
	}
}
