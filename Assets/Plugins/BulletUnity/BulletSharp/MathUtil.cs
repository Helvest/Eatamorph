...
namespace BulletSharp
{
	public static class MathUtil
	{
		...
		{
			var c = Vector3.Cross(axisInA, axisInB);
			Vector3.Dot(ref axisInA, ref axisInB, out float d);
		...
		{
			var q = QuatVectorMultiply(ref rotation, ref v);
        ...
        */
		...
		{
			if (System.Math.Abs(v.X) > 1e-6 || System.Math.Abs(v.Y) > 1e-6 || System.Math.Abs(v.Z) > 1e-6)
			{
				return false;
			}
		...
		{
			var xa = new Vector3(x.X, 0, x.Z);
			var ya = new Vector3(y.X, 0, y.Z);
