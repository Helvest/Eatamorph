...
namespace BulletSharp
{
	public class KinematicCharacterController : ICharacterController
	{
		...
		{
			m_convexShape.GetAabb(m_ghostObject.WorldTransform, out var minAabb, out var maxAabb);
			...
			for (int i = 0; i < m_ghostObject.OverlappingPairCache.NumOverlappingPairs; i++)
			{
				...
				{
					var manifold = m_manifoldArray[j];
			...
			var newTrans = m_ghostObject.WorldTransform;
		...
		{
			var movementDirection = m_targetPosition - m_currentPosition;
			float movementLength = movementDirection.Length;
			if (movementLength > MathUtil.SIMD_EPSILON)
			{
				movementDirection.Normalize();

				var reflectDir = ComputeReflectionDirection(ref movementDirection, ref hitNormal);
				reflectDir.Normalize();

				//Vector3 parallelDir = 
				ParallelComponent(ref reflectDir, ref hitNormal);

				var perpindicularDir = PerpindicularComponent(ref reflectDir, ref hitNormal);
                ...
                */
				if (normalMag != 0.0f)
				{
					var perpComponent = perpindicularDir * (normalMag * movementLength);
		...
		protected void StepForwardAndStrafe(CollisionWorld collisionWorld, ref Vector3 walkMove)
		{
			...
			{
				Vector3.Dot(ref m_normalizedDirection, ref m_touchingNormal, out float dot);
				if (dot > 0.0f)
				{
			...
			while (fraction > 0.01f && maxIter-- > 0)
			{
				...
				var sweepDirNegative = m_currentPosition - m_targetPosition;

				var callback = new KinematicClosestNotMeConvexResultCallback(m_ghostObject, sweepDirNegative, 0f)
				{
					CollisionFilterGroup = GhostObject.BroadphaseHandle.CollisionFilterGroup,
					CollisionFilterMask = GhostObject.BroadphaseHandle.CollisionFilterMask
				};
				...
				if (callback.HasHit)
				{
					...
					var hitNormalWorld = callback.HitNormalWorld;
					UpdateTargetPositionBasedOnCollision(ref hitNormalWorld, 0f, 1f);
					var currentDir = m_targetPosition - m_currentPosition;
					distance2 = currentDir.LengthSquared;
					if (distance2 > MathUtil.SIMD_EPSILON)
					{
						currentDir.Normalize();
						/* See Quake2: "If velocity is against original velocity, stop ead to avoid tiny oscilations in sloping corners." */
						Vector3.Dot(ref currentDir, ref m_normalizedDirection, out float dot);
						if (dot <= 0.0f)
						{
							break;
						}
		...
		protected void StepDown(CollisionWorld collisionWorld, float dt)
		{
			...
			var orig_position = m_targetPosition;
			...
			var step_drop = upAxisDirection[m_upAxis] * (m_currentStepOffset + downVelocity);
			m_targetPosition -= step_drop;

			var callback = new KinematicClosestNotMeConvexResultCallback(m_ghostObject, upAxisDirection[m_upAxis], m_maxSlopeCosine)
			{
				CollisionFilterGroup = GhostObject.BroadphaseHandle.CollisionFilterGroup,
				CollisionFilterMask = GhostObject.BroadphaseHandle.CollisionFilterMask
			};

			var callback2 = new KinematicClosestNotMeConvexResultCallback(m_ghostObject, upAxisDirection[m_upAxis], m_maxSlopeCosine)
			{
				CollisionFilterGroup = GhostObject.BroadphaseHandle.CollisionFilterGroup,
				CollisionFilterMask = GhostObject.BroadphaseHandle.CollisionFilterMask
			};
			...
			while (true)
			{
							...
							&& (m_wasOnGround || !m_wasJumping))
				{
					...
					var step_drop2 = upAxisDirection[m_upAxis] * (m_currentStepOffset + downVelocity);
		...
		public void Reset(CollisionWorld collisionWorld)
		{
			...
			//clear pair cache
			var cache = m_ghostObject.OverlappingPairCache;
		...
		public void PlayerStep(CollisionWorld collisionWorld, float dt)
		{
			...
			else
			{
				...
				var move = m_walkDirection * dtMoving;
		...
		public bool OnGround => m_verticalVelocity == 0.0f && m_verticalOffset == 0.0f;
	...
	public class KinematicClosestNotMeConvexResultCallback : ClosestConvexResultCallback
	{
		...
		public override float AddSingleResult(LocalConvexResult convexResult, bool normalInWorldSpace)
		{
			...
			Vector3.Dot(ref _up, ref hitNormalWorld, out float dotUp);
