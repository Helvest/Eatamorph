...
namespace BulletSharp
{
	...
	{
		private const int SizeOfBlenderHeader = 12;
        ...
        */
		...
		{
			if (LibPointers.TryGetValue(ptr, out byte[] data))
			{
				return data;
			}
			return null;
		...
		{
			foreach (var element in firstStruct.Elements)
			{
		...
		protected void GetMatchingFileDna(Dna.StructDecl dna, Dna.ElementDecl lookupElement, BinaryWriter strcData, long data, bool fixupPointers)
		{
			...
			var dataStream = new MemoryStream(_fileBuffer, false);
			var dataReader = new BinaryReader(dataStream);

			foreach (var element in dna.Elements)
			{
		...
		protected int GetNextBlock(out ChunkInd dataChunk, BinaryReader reader, FileFlags flags)
		{
			...
			if (IntPtr.Size == 8)
			{
				if (varies)
				{
					var c = new ChunkPtr4(reader);
					dataChunk = new ChunkInd(ref c);
				}
				else
				{
					var c = new ChunkPtr8(reader);
					dataChunk = new ChunkInd(ref c);
				}
			}
			else
		...
		public bool OK => (_flags & FileFlags.OK) != 0;
		...
		protected void ParseInternal(FileVerboseMode verboseMode, byte[] memDna)
		{
			...
			var dna = new ChunkInd
			{
				OldPtr = 0
			};

			var memory = new MemoryStream(_fileBuffer, false);
			...
			_memoryDna = new Dna();
			using (var memory2 = new MemoryStream(memDna, false))
			{
				using (var reader2 = new BinaryReader(memory2))
				{
		...
		protected void ParseStruct(BinaryWriter strc, BinaryReader data, Dna.StructDecl fileStruct, Dna.StructDecl memoryStruct, bool fixupPointers)
		{
			...
			foreach (var element in memoryStruct.Elements)
			{
				int memorySize = _memoryDna.GetElementSize(element);
				if (element.Type.Struct != null && element.Name.Name[0] != '*')
				{
					long elementOffset = GetFileElement(fileStruct, element, dataPtr, out var elementOld);
					if (elementOffset != 0)
					{
						var oldStruct = _fileDna.GetStruct(_fileDna.GetReverseType(element.Type.Name));
		...
		protected byte[] ReadStruct(BinaryReader head, ChunkInd dataChunk)
		{
			...
			if (!_fileDna.FlagEqual(dataChunk.DnaNR))
			{
				...
				var oldStruct = _fileDna.GetStruct(dataChunk.DnaNR);
				...
				else
				{
					...
					if (reverseOld != -1)
					{
						var curStruct = _memoryDna.GetStruct(reverseOld);
		...
		{
			var fileDna = (_fileDna != null) ? _fileDna : _memoryDna;
			...
			foreach (var dataChunk in _chunks)
			{
				if (_fileDna == null || fileDna.FlagEqual(dataChunk.DnaNR))
				{
					var oldStruct = fileDna.GetStruct(dataChunk.DnaNR);
		...
		{
			var fileDna = (_fileDna != null) ? _fileDna : _memoryDna;
			var oldStruct = fileDna.GetStruct(dnaNr);

			int totalSize = 0;

			foreach (var element in oldStruct.Elements)
			{
        ...
        */
		public FileFlags Flags => _flags;

		public Dictionary<long, byte[]> LibPointers => _libPointers;
