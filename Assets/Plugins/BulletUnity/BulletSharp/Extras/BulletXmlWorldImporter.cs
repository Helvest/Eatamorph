...
namespace BulletSharp
{
	public class BulletXmlWorldImporter : WorldImporter
	{
		...
		private void AutoSerializeRootLevelChildren(XmlElement parent)
		{
			...
			{
				var shape = ConvertCollisionShape(shapeData, _pointerLookup);
				if (shape != null)
				{
					foreach (var lib in _pointerLookup)
					{
		...
		private void DeSerializeCompoundShapeChildData(XmlElement element)
		{
			...
			var transforms = element.SelectNodes("m_transform");
			var childShapes = element.SelectNodes("m_childShape");
			var childShapeTypes = element.SelectNodes("m_childShapeType");
			var childMargins = element.SelectNodes("m_childMargin");
		...
		{
			if (!int.TryParse(element.GetAttribute("pointer"), out int ptr))
			{
				_fileOK = false;
				return;
			}
		...
		private void DeSerializeVector3FloatData(XmlElement element)
		{
			...
			var vectors = element.SelectNodes("m_floats");
		...
		public bool LoadFile(string fileName)
		{
			...
			var bulletPhysics = doc.DocumentElement;
