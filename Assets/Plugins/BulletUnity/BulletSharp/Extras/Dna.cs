...
namespace BulletSharp
{
	public class Dna
	{
		...
		public class NameInfo
		{
			...
			public int ArraySizeNew => Dim0 * Dim1;
		...
		{
			if (_structReverse.TryGetValue(typeName, out int s))
			{
				return s;
			}
			return -1;
		...
		public void InitCmpFlags(Dna memoryDna)
		{
			...
			for (int i = 0; i < _structs.Length; i++)
			{
				...
				{
					var curStruct = memoryDna.GetStruct(oldLookup);
		...
		private void InitRecurseCmpFlags(Dna.StructDecl iter)
		{
			for (int i = 0; i < _structs.Length; i++)
			{
				var curStruct = _structs[i];
				if (curStruct != iter && _cmpFlags[i] == FileDnaFlags.StructEqual)
				{
					foreach (var element in curStruct.Elements)
					{
