...
namespace BulletSharp
{
	public class WorldImporter
	{
		...
		protected CollisionShape ConvertCollisionShape(byte[] shapeData, Dictionary<long, byte[]> libPointers)
		{
			...
			var stream = new MemoryStream(shapeData, false);
			var reader = new BulletReader(stream);

			var type = (BroadphaseNativeType)reader.ReadInt32(CollisionShapeFloatData.Offset("ShapeType"));
			stream.Position = Marshal.SizeOf(typeof(CollisionShapeFloatData));
			switch (type)
			{
				case BroadphaseNativeType.StaticPlaneShape:
				{
					var localScaling = reader.ReadVector3();
				...
				case BroadphaseNativeType.GImpactShape:
				{
					...
					var meshInterface = CreateMeshInterface(shapeData,
						GImpactMeshShapeData.Offset("MeshInterface"), libPointers);

					var gImpactType = (GImpactShapeType)reader.ReadInt32(GImpactMeshShapeData.Offset("GImpactSubType"));
					if (gImpactType == GImpactShapeType.TrimeshShape)
					{
						var gimpactShape = CreateGimpactShape(meshInterface);
				...
				case BroadphaseNativeType.CompoundShape:
				{
					...
					var compoundShape = CreateCompoundShape();
					using (var shapeStream = new MemoryStream(childShapes, false))
					{
						using (var shapeReader = new BulletReader(shapeStream))
						{
							...
							{
								var localTransform = shapeReader.ReadMatrix();
								...
								var childShape = ConvertCollisionShape(libPointers[childShapePtr], libPointers);
				...
				{
					var localScaling = reader.ReadVector3();
					var implicitShapeDimensions = reader.ReadVector3();
					...
					switch (type)
					{
						case BroadphaseNativeType.BoxShape:
						{
							var boxExtents = implicitShapeDimensions / localScaling + new Vector3(collisionMargin);
							var box = CreateBoxShape(ref boxExtents) as BoxShape;
						...
						case BroadphaseNativeType.CapsuleShape:
						{
							var halfExtents = implicitShapeDimensions + new Vector3(collisionMargin);
						...
						case BroadphaseNativeType.ConvexHullShape:
						{
							...
							var hullShape = CreateConvexHullShape();
						...
						case BroadphaseNativeType.MultiSphereShape:
						{
							...
							var positions = new Vector3[localPositionArraySize];
				...
				case BroadphaseNativeType.TriangleMeshShape:
				{
					...
					var trimeshShape = CreateBvhTriangleMeshShape(meshInterface, bvh);
		...
		protected void ConvertConstraintFloat(RigidBody rigidBodyA, RigidBody rigidBodyB, byte[] constraintData, int fileVersion, Dictionary<long, byte[]> libPointers)
		{
			...
			var type = (TypedConstraintType)reader.ReadInt32(TypedConstraintFloatData.Offset("ObjectType"));
			switch (type)
			{
				case TypedConstraintType.Point2Point:
				{
					var pivotInA = reader.ReadVector3(Point2PointConstraintFloatData.Offset("PivotInA"));
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var pivotInB = reader.ReadVector3(Point2PointConstraintFloatData.Offset("PivotInB"));
				...
				case TypedConstraintType.ConeTwist:
				{
					...
					var rbaFrame = reader.ReadMatrix(ConeTwistConstraintFloatData.Offset("RigidBodyAFrame"));
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbbFrame = reader.ReadMatrix(ConeTwistConstraintFloatData.Offset("RigidBodyBFrame"));
				...
				case TypedConstraintType.D6:
				{
					...
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbaFrame = reader.ReadMatrix(Generic6DofConstraintFloatData.Offset("RigidBodyAFrame"));
						var rbbFrame = reader.ReadMatrix(Generic6DofConstraintFloatData.Offset("RigidBodyBFrame"));
				...
				case TypedConstraintType.D6Spring:
				{
					...
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbaFrame = reader.ReadMatrix(sixDofData + Generic6DofConstraintFloatData.Offset("RigidBodyAFrame"));
						var rbbFrame = reader.ReadMatrix(sixDofData + Generic6DofConstraintFloatData.Offset("RigidBodyBFrame"));
				...
				case TypedConstraintType.D6Spring2:
				{
					...
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbaFrame = reader.ReadMatrix(Generic6DofSpring2ConstraintFloatData.Offset("RigidBodyAFrame"));
						var rbbFrame = reader.ReadMatrix(Generic6DofSpring2ConstraintFloatData.Offset("RigidBodyBFrame"));
						var rotateOrder = (RotateOrder)reader.ReadInt32(Generic6DofSpring2ConstraintFloatData.Offset("RotateOrder"));
				...
				case TypedConstraintType.Gear:
				{
					...
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var axisInA = reader.ReadVector3(GearConstraintFloatData.Offset("AxisInA"));
						var axisInB = reader.ReadVector3(GearConstraintFloatData.Offset("AxisInB"));
				...
				case TypedConstraintType.Hinge:
				{
					...
					var rbaFrame = reader.ReadMatrix(HingeConstraintFloatData.Offset("RigidBodyAFrame"));
					int useReferenceFrameA = reader.ReadInt32(HingeConstraintFloatData.Offset("UseReferenceFrameA"));
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbbFrame = reader.ReadMatrix(HingeConstraintFloatData.Offset("RigidBodyBFrame"));
				...
				case TypedConstraintType.Slider:
				{
					...
					var rbbFrame = reader.ReadMatrix(SliderConstraintFloatData.Offset("RigidBodyBFrame"));
					int useLinearReferenceFrameA =
						reader.ReadInt32(SliderConstraintFloatData.Offset("UseLinearReferenceFrameA"));
					if (rigidBodyA != null && rigidBodyB != null)
					{
						var rbaFrame = reader.ReadMatrix(SliderConstraintFloatData.Offset("RigidBodyAFrame"));
		...
		{
			var stream = new MemoryStream(bodyData, false);
			var reader = new BulletReader(stream);
			...
			var startTransform = reader.ReadMatrix(cod + CollisionObjectFloatData.Offset("WorldTransform"));
			...
			var shape = _shapeMap[collisionShapePtr];
			...
			var body = CreateRigidBody(isDynamic, mass, ref startTransform, shape, name);
		...
		{
			var shape = new CapsuleShapeZ(radius, height);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var shape = new CapsuleShapeX(radius, height);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var shape = new ConeShape(radius, height);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var constraint = new ConeTwistConstraint(rbA, rbAFrame);
		...
		{
			var constraint = new ConeTwistConstraint(rbA, rbB, rbAFrame, rbBFrame);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
        ...
        */
		...
		{
			var shape = new CylinderShapeZ(radius, radius, height);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var shape = new CylinderShapeX(height, radius, radius);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var shape = new CylinderShape(radius, height, radius);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var constraint = new GearConstraint(rbA, rbB, axisInA, axisInB, ratio);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new Generic6DofConstraint(rbB, frameInB, useLinearReferenceFrameB);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new Generic6DofConstraint(rbA, rbB, frameInA, frameInB, useLinearReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new Generic6DofSpringConstraint(rbA, rbB, frameInA, frameInB, useLinearReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new Generic6DofSpring2Constraint(rbA, rbB, frameInA, frameInB, rotateOrder);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var shape = new GImpactMeshShape(trimesh._native);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var constraint = new HingeConstraint(rbA, rbB, rbAFrame, rbBFrame, useReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new HingeConstraint(rbA, rbB, rbAFrame, rbBFrame);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new HingeConstraint(rbA, rbAFrame, useReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new HingeConstraint(rbA, rbAFrame);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var meshInterface = CreateTriangleMeshContainer();
			byte[] meshParts;
			//Vector3 scaling;
			int numMeshParts;
		...
		{
			var shape = new MultiSphereShape(positions, radi);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var shape = new StaticPlaneShape(planeNormal, planeConstant);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var constraint = new Point2PointConstraint(rbA, rbB, pivotInA, pivotInB);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new Point2PointConstraint(rbA, pivotInA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		public virtual RigidBody CreateRigidBody(bool isDynamic, float mass, ref Matrix startTransform, CollisionShape shape, string bodyName)
		{
			...
			var info = new RigidBodyConstructionInfo(mass, null, shape, localInertia);
		...
		{
			var shape = new ScaledBvhTriangleMeshShape(meshShape, localScalingbtBvhTriangleMeshShape);
			_allocatedCollisionShapes.Add(shape);
			return shape;
		}
		...
		{
			var constraint = new SliderConstraint(rbB, frameInB, useLinearReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var constraint = new SliderConstraint(rbA, rbB, frameInA, frameInB, useLinearReferenceFrameA);
			_allocatedConstraints.Add(constraint);
			return constraint;
		}
		...
		{
			var tiva = new TriangleIndexVertexArray();
		...
		{
			foreach (var constraint in _allocatedConstraints)
			{
				if (_dynamicsWorld != null)
				{
			...
			foreach (var rigidBody in _allocatedRigidBodies)
			{
				if (_dynamicsWorld != null)
				{
			...
			foreach (var shape in _allocatedCollisionShapes)
			{
				shape.Dispose();
			}
			_allocatedCollisionShapes.Clear();

			foreach (var bvh in _allocatedBvhs)
			{
				bvh.Dispose();
			}
			_allocatedBvhs.Clear();

			foreach (var tim in _allocatedTriangleInfoMaps)
			{
				tim.Dispose();
			}
			_allocatedTriangleInfoMaps.Clear();

			foreach (var tiva in _allocatedTriangleIndexArrays)
			{
				tiva.Dispose();
			}
		...
		{
			_nameShapeMap.TryGetValue(name, out var shape);
			return shape;
		}
		...
		{
			_nameConstraintMap.TryGetValue(name, out var constraint);
			return constraint;
		}

		public string GetNameForObject(object obj)
		{
			_objectNameMap.TryGetValue(obj, out string name);
			return name;
		}
		...
		{
			_nameBodyMap.TryGetValue(name, out var body);
			return body;
		}
