...
namespace BulletSharp
{
	public abstract class MotionState : IDisposable
	{
		...
		private delegate void GetWorldTransformUnmanagedDelegate(IntPtr managedMotionStatePtr, out Matrix worldTrans);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void SetWorldTransformUnmanagedDelegate(IntPtr managedMotionStatePtr, ref Matrix worldTrans);
		...
		private static void GetWorldTransformUnmanaged(IntPtr msPtr, out Matrix worldTrans)
		{
			//UnityEngine.Debug.Log("Get" + msPtr.ToInt64());
			var ms = GCHandle.FromIntPtr(msPtr).Target as MotionState;
			ms.GetWorldTransform(out worldTrans);
		}

		[MonoPInvokeCallback(typeof(SetWorldTransformUnmanagedDelegate))]
		private static void SetWorldTransformUnmanaged(IntPtr msPtr, ref Matrix worldTrans)
		{
		...
		public Matrix WorldTransform
		{
			get
			{
				GetWorldTransform(out var transform);
				return transform;
			}
		...
		[DllImport(Native.Dll, CallingConvention = Native.Conv), SuppressUnmanagedCodeSecurity]
		private static extern void btMotionState_delete(IntPtr obj);

		[DllImport(Native.Dll, CallingConvention = Native.Conv), SuppressUnmanagedCodeSecurity]
		private static extern IntPtr btMotionStateWrapper_new(IntPtr getWorldTransformCallback, IntPtr setWorldTransformCallback, IntPtr motionStatePtr);
	}
}
