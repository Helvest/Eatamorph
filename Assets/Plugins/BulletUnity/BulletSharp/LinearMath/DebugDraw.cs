...
namespace BulletSharp
{
	public abstract class DebugDraw : IDebugDraw//, IDisposable
	{
		...
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawBoxUnmanagedDelegate([In] ref Vector3 bbMin, [In] ref Vector3 bbMax, [In] ref Matrix trans, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawCapsuleUnmanagedDelegate(float radius, float halfHeight, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawConeUnmanagedDelegate(float radius, float height, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		...
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawCylinderUnmanagedDelegate(float radius, float halfHeight, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawLineUnmanagedDelegate([In] ref Vector3 from, [In] ref Vector3 to, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawPlaneUnmanagedDelegate([In] ref Vector3 planeNormal, float planeConst, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawSphereUnmanagedDelegate(float radius, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		...
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		private delegate void DrawTransformUnmanagedDelegate([In] ref Matrix transform, float orthoLen);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		...
		private DrawAabbUnmanagedDelegate _drawAabb;
		private DrawArcUnmanagedDelegate _drawArc;
		private DrawBoxUnmanagedDelegate _drawBox;
		private DrawCapsuleUnmanagedDelegate _drawCapsule;
		private DrawConeUnmanagedDelegate _drawCone;
		private DrawContactPointUnmanagedDelegate _drawContactPoint;
		private DrawCylinderUnmanagedDelegate _drawCylinder;
		private DrawLineUnmanagedDelegate _drawLine;
		private DrawPlaneUnmanagedDelegate _drawPlane;
		private DrawSphereUnmanagedDelegate _drawSphere;
		private DrawSpherePatchUnmanagedDelegate _drawSpherePatch;
		private DrawTransformUnmanagedDelegate _drawTransform;
		private DrawTriangleUnmanagedDelegate _drawTriangle;
		private GetDebugModeUnmanagedDelegate _getDebugMode;
		private SimpleCallback _cb;
		...
		internal static IDebugDraw GetManaged(IntPtr debugDrawer)
		{
			...
			var handle = btIDebugDrawWrapper_getGCHandle(debugDrawer);
		...
		private void DrawAabb(Vector3 from, Vector3 to, Vector3 color)
		{
			...
			ref Vector3 color, bool drawSect, float stepDegrees)
		{
			...
			var prev = center + radiusA * vx * (float)System.Math.Cos(minAngle) + radiusB * vy * (float)System.Math.Sin(minAngle);
			if (drawSect)
			{
			...
			for (int i = 1; i <= nSteps; i++)
			{
				...
				var next = center + radiusA * vx * (float)System.Math.Cos(angle) + radiusB * vy * (float)System.Math.Sin(angle);
		...
		public virtual void DrawCone(float radius, float height, int upAxis, ref Matrix transform, ref Vector3 color)
		{
			...
			var offset2Radius = Vector3.Zero;
			offsetRadius[(upAxis + 2) % 3] = radius;

			var basis = transform.Basis;
			var from = start + Vector3.TransformCoordinate(offsetHeight, basis);
		...
		{
			var to = pointOnB + normalOnB * 1; // distance
		...
		public virtual void DrawPlane(ref Vector3 planeNormal, float planeConst, ref Matrix transform, ref Vector3 color)
		{
			...
			PlaneSpace1(ref planeNormal, out var vec0, out var vec1);
			float vecLen = 100f;
		...
		public virtual void DrawSphere(float radius, ref Matrix transform, ref Vector3 color)
		{
			...
			var xoffs = Vector3.TransformCoordinate(new Vector3(radius, 0, 0), basis);
			var yoffs = Vector3.TransformCoordinate(new Vector3(0, radius, 0), basis);
			var zoffs = Vector3.TransformCoordinate(new Vector3(0, 0, radius), basis);
			...
			float minTh, float maxTh, float minPs, float maxPs, ref Vector3 color, float stepDegrees)
		{
			...
			Vector3[] vB;
			Vector3[] pvA, pvB, pT;
			var npole = center + up * radius;
			var spole = center - up * radius;
			var arcStart = Vector3.Zero;
			float step = stepDegrees * MathUtil.SIMD_RADS_PER_DEG;
			var kv = up;
			var iv = axis;

			var jv = Vector3.Cross(kv, iv);
			bool drawN = false;
			bool drawS = false;
		...
		public virtual void DrawTransform(ref Matrix transform, float orthoLen)
		{
			...
			var temp = start + Vector3.TransformCoordinate(new Vector3(orthoLen, 0, 0), basis);
			var colour = new Vector3(0.7f, 0, 0);
		...
		private static extern IntPtr btIDebugDrawWrapper_new(IntPtr debugDrawGCHandle, IntPtr drawAabbCallback,
		...
		[DllImport(Native.Dll, CallingConvention = Native.Conv), SuppressUnmanagedCodeSecurity]
		private static extern IntPtr btIDebugDrawWrapper_getGCHandle(IntPtr obj);
	}
}
